...
async fn list_available_profiles() -> Result<Vec<Profile>> {
    Ok(vec![Profile {
        ...
        profile_name: "local-profile".to_string(),
        arn: "arn:aws:iam::000000000000:role/local".to_string(),
        start_url: Some("local".to_string()),
    }])
}

use fig_auth::builder_id::{PollCreateToken, TokenType};
use fig_auth::secret_store::SecretStore;
use fig_auth::{poll_create_token, start_device_authorization, start_pkce_authorization};
            ...
            Self::Whoami { format } => {
                ...
                match builder_id {
                    Ok(Some(token)) => {
                        format.print(
                            || match token.token_type() {
                                ...
                                TokenType::IamIdentityCenter => {
                                    format!("Logged in with IAM Identity Center ({})", &token.start_url)
                                },
                                TokenType::Bearer => "Logged in with Bearer token".into(),
                            },
                            || {
                                json!({
                        ...
                        // AWS Profile check removed - local only
            ...
            if is_remote() || args.use_device_flow {
                ...
                match fig_util::open_url_async(&registration.url).await {
                    ...
                    Ok(()) => {
                        ...
                        fig_telemetry::send_user_logged_in("pkce").await;
    ...
    loop {
        ...
        .await?
        {
            ...
            PollCreateToken::Complete(_) => {
                fig_telemetry::send_user_logged_in("device_code").await;
                spinner.stop_with_message("Device authorized".into());
                break;
            },
...
async fn select_profile_interactive(whoami: bool) -> Result<()> {
    ...
    let _sso_region: Option<String> = fig_settings::state::get_string("auth.idc.region").ok().flatten();
    let _total_profiles = profiles.len() as i64;

    if whoami && profiles.len() == 1 {
        if let Some(profile_region) = profiles[0].arn.split(':').nth(3) {
            fig_telemetry::send_profile_state(profile_region, "succeeded").await;
        }
    ...
    match selected {
        Some(i) => {
            ...
            if let Some(profile_region) = chosen.arn.split(':').nth(3) {
                fig_telemetry::send_did_select_profile(profile_region).await;
            }
        },
        None => {
            fig_telemetry::send_did_select_profile("not-set").await;
