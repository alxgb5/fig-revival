...
use feed::Feed;
use fig_auth::builder_id::{BuilderIdToken, DeviceRegistration, Region};
...
use fig_telemetry_core::TelemetryResult;
...
use crate::util::{CliContext, assert_logged_in, qchat_path};
...
impl Cli {
            ...
            if let Ok(database) = database().map_err(|err| error!(?err, "failed to open database")) {
                if let Ok(token) = BuilderIdToken::load(&secret_store, false).await {
                    ...
                    if let Some(token) = token.as_ref() {
                        let region = Region::new(&token.region);
    ...
    async fn send_telemetry(&self) {
        match &self.subcommand {
            ...
            Some(subcommand) => {
                fig_telemetry::send_cli_subcommand_executed(subcommand.name(), TelemetryResult::Succeeded).await;
            },
        }
    }
